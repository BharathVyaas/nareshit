  ...
  return (
    ...
    >
      {questions.length > 0 ?  (
        ...
        />
      ) : <div className="h-20 grid place-content-center"><p className="font-bold text-gray-400">No Question To Show!</p></div>}
    </motion.div>
  );
}
...
function QuestionViewFixedModalFooter({
  ...
  const submitHandler = async () => {
    ...
    console.log('current',obj, key)

    if(natureId === 2 || natureId === '2')
    console.log('Transformed array:', Object.values(data.combination).map(ele => {
      if (ele.id === key) {
          return { easy: Number(obj?.easy) || 0, medium: Number(obj?.medium) || 0, hard: Number(obj?.hard) || 0 };
      }
