...
console.log(natureID)
  return (
    <>{showIncludedModal && <ShowIncludedModalHelper data={{}} showIncludedFn={showIncludedFn} handler={showIncludedModal} />}
    <div className="p-5">
      ...
      )}</div>{(natureID === '2' || natureID === 2) && <div>
        <button className="mr-[20px] mb-3 w-[15rem] px-6 max-h-8 min-h-8 bg-[gray] text-white font-semibold rounded-md shadow-md hover:bg-gray-500 focus:outline-none focus:ring-2 focus:bg-gray-500 focus:ring-opacity-50" onClick={showIncludedFn}>Show Included Questions</button>
      </div>}</div>
      <div className="">
...
function ShowIncludedModalHelper({data
  ,showIncludedFn
  ,handler}){
  return <Modal data={data} setter={showIncludedFn} ModalParam={ShowIncludeModal} handler={handler} />
}