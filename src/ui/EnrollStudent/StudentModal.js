  ...
  return (
    <div className="p-4">
      ...
      <table className="pl-6 max-h-80 overflow-y-auto border rounded-lg p-2">
        ...
        <tbody> 
            {data.modalData.map(student => <tr key={student.StudentID} className="border-b border-gray-300">
            <td className="px-1"><FormControlLabel
            ...
            <td className="px-1">{student.CreatedAt ? new Date(student.CreatedAt).toLocaleDateString() : ''}</td>
            </tr>)}
        </tbody>
      </table>
      ...
      <div className="text-center mt-4">
        <Button variant="contained" sx={{width: '8rem'}} color="primary" onClick={() => {
            handler(excludedArr)
            setter(false)
            }}>
          Submit
