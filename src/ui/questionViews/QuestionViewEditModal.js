...
function QuestionViewEditModal({ data, handler, setter }) {
  ...
  useEffect(() => {
    ...
    if (Number(maxEasyCount) === 0) setCorrectEasy(0);
      ...
      setCorrectEasy(
        maxEasyCount >= availableEasyTotal ? queryEasy - easyTotal >= maxEasyCount ? maxEasyCount : queryEasy - easyTotal : maxEasyCount
      );
    }
    ...
    if (Number(maxMediumCount) === 0) {
      ...
      setCorrectMedium(
        maxMediumCount >= availableMediumTotal
          ? queryMedium - mediumTotal >= maxMediumCount ? maxMediumCount : queryMedium - mediumTotal
          : maxMediumCount
      );
    }
    ...
    if (Number(maxHardCount) === 0) {
      ...
      setCurrentHard(
        maxHardCount >= availableHardTotal
          ? queryHard - hardTotal >= maxHardCount ? maxHardCount : queryHard - hardTotal
          : maxHardCount
      );
    }
