...
function Topics({ setDataHandler, combination }) {
  ...
  // Topics
  useEffect(() => {
    if (selectedModule && (selectedTopic !== -1 && selectedTopic !== '-1')) fetchTopics(selectedModule);
  }, [selectedModule]);
  ...
  // SubTopics
  useEffect(() => {
    if (selectedTopic && (selectedSubTopic !== -1 && selectedSubTopic !== '-1')) fetchSubTopics();
  }, [selectedTopic]);
      ...
      if (obj.length == 0) {
        ...
        obj.forEach((ele) => {
          if (!valid) {
            ...
            selectedModule == ele.ModuleID &&
            selectedTopic == ele.TopicID &&
            (selectedSubTopic == 0 || selectedSubTopic == -1) && !valid
          ) {
